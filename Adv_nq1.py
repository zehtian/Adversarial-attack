...
def loss_sin(model, adv_list, x_adv, x_ori, adv_label, FL_params):
    ...
    * 把向量转成1维，求相似度
    ...
    # 两轮对抗样本连线的向量
    xadv_sub = adv_list[0] - adv_list[1]
    xadv_sub = xadv_sub.view(-1)
    xadv_sub = xadv_sub / torch.norm(xadv_sub, p=FL_params.attack_norm)

    # 求余弦相似度
    sin = torch.abs(torch.cosine_similarity(x_sub, xadv_sub, dim=-1))

    # 此部分代码用于调试
    print('d0(x_ori->x_adv):', torch.norm(x_adv - x_ori, p=FL_params.attack_norm).item())
    print('d1(x_i->x_i-1):', torch.norm(adv_list[0] - adv_list[1], p=FL_params.attack_norm).item())
    print('cos(d0~d1):', torch.cosine_similarity(x_sub, xadv_sub, dim=-1).item())
...
def adv_attack1(model, x_sample, y_sample, tgt_sample, adv_label, FL_params):
    """
    黑盒对抗攻击主函数
    * 1.在决策边界上随机生成一个对抗样本；
    ...
    * 注意：model，x_sample，y_sample，tgt_sample 均在 FL_params.device 上
    ...
    for attack_epoch in range(FL_params.attack_iter):
        ...
        # 损失函数梯度计算
        # print('d(x_i->x_i-1):', torch.norm(adv_list[0] - adv_list[1]).item())
        ...
        if FL_params.opt_mode == 'MOM3':
            ...
            if not attack_epoch:
                ...
                l = (adv_list[0] - adv_list[1]) / torch.norm(adv_list[0] - adv_list[1], p=FL_params.attack_norm)
            momentum = beta1 * momentum + (1 - beta1) * l
            update = beta2 * momentum + (1 - beta2) * grads
            adv_sample -= lr * update
        ...
        # 注意，loss为上一轮决策边界上点的loss
        print("adv_ours: epoch:{}, adv_label:{}, loss:{}, distance:{}".
              format(attack_epoch+1, torch.argmax(model(adv_sample)), loss, distance_value))
