...
from ToolBox.art.attacks.evasion import HopSkipJump
from ToolBox.art.utils import to_categorical
from Adv_ours import binary_search

def hsja_initialize(model, x_sample, y_sample, tgt_sample, adv_label, FL_params):
    """
    * 采用hsja攻击，在决策边界处初始化一个对抗样本
    ...
    attack = HopSkipJump(classifier=M_tgt, batch_size=FL_params.train_batch_size, targeted=FL_params.target_attack, max_iter=FL_params.attack_iter, init_eval=FL_params.num_evals_boundary)
    ...
    x_adv = binary_search(model, x_sample, x_adv, adv_label, FL_params)

    d = torch.norm(x_adv - x_sample, p=FL_params.attack_norm)
    adv_label = model(x_adv).argmax(-1)
    print("hsja attack: ori_label:{}, adv_label:{}, distance:{}".format(y_sample, adv_label, d))
